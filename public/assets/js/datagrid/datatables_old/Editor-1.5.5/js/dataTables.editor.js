 ...
 *    } );
	...
	Editor.prototype.edit = function ( items, arg1, arg2, arg3, arg4 )
	{
		...
		this._edit(items, this._dataSource('fields', items), 'main', arg2);
...
// Global listener for file information updates via DataTables' Ajax JSON
	 ...
	 * @private
	 */
	Editor.prototype._edit = function (items, editFields, type, myObj)
	{
		var that = this;
		var fields = this.s.fields;
		var usedFields = [];
		...
		$.each( fields, function ( name, field ) {
			...
			$.each( editFields, function ( idSrc, edit ) {
				if ( edit.fields[ name ] ) {
					...
					if ($(edit.fields[ name ].dom.inputControl[0].childNodes[0]).is('select')) {
						if (field.valFromData(edit.data) && $(edit.fields[ name ].dom.inputControl[0].childNodes[0]).attr('dependent-group-level') == "1") {
							myObj.populateQueryLists($(edit.fields[name].dom.inputControl[0].childNodes[0]), field.valFromData(edit.data), that);
						}
						if (field.valFromData(edit.data) && $(edit.fields[ name ].dom.inputControl[0].childNodes[0]).attr('dependent-group-level') != "1" && $(edit.fields[ name ].dom.inputControl[0].childNodes[0]).attr('dependent-group-level') != undefined) {
							myObj.dependentListsShowMode($(edit.fields[name].dom.inputControl[0].childNodes[0]), field.valFromData(edit.data), that);
						}
						if ($(edit.fields[name].dom.inputControl[0].childNodes[0]).attr('dependent-group-level') == myObj.formDependentGroupData.length + 1) {
