...
debugger
            if (id === undefined) {
                ...
                // Common value
                //QUADSYSTEMS
		if ($("[id='" + this.s.opts.id + "']").is('select')) {
                    value = $("[id='" + this.s.opts.id + "']").val();
//                if ($("#" + this.s.opts.id).is('select')) {
//                    value = $("#" + this.s.opts.id).val();
                } else {
                    //value = this.val();
                    value = $("[id='" + this.s.opts.id + "']").val();
                }
...
// Global listener for file information updates via DataTables' Ajax JSON
     ...
     *     been uploaded
     */
    Editor.upload = function (editor, conf, files, progressCallback, completeCallback, container) {
        ...
        progressCallback(conf, conf.fileReadText || "<i>Adicionar outro</i>");
        if (editor.frmData) {
            //editor.frmData.append( 'upload[]', files[ counter ] );
        } else {
        ...
        reader.onload = function (e) {
            ...
            var image = "<image style='width:100px;heigth:inherit;' src='" + e.target.result + "'/>";
            container.append(image);

            var ajax;

            if (conf.ajaxData) {
                conf.ajaxData(editor.frmData);
            }
            if (conf.ajax) {
                ajax = conf.ajax;
            }
    ...
    Editor.prototype._constructor = function (init) {
        ...
        this.dom = {
            ...
            "header": $('<div data-dte-e="head" class="' + classes.header.wrapper + '"><div class="' + classes.header.content +
                '"/>' +
                '<div class="progress">' +
...
// Upload private helper method
        function _buttonText(conf, text) {
            if (text === null || text === undefined) {
                text = conf.uploadText || JS_CHOOSE_FILE + "...";
            }
        ...
        function _commonUpload(editor, conf, dropCallback) {
            ...
            var container = $(
                ...
                //'<button class="'+btnClass+'" /> Upload</button>'+
                '<button class="' + btnClass + '" /> </button>' +
                '<input type="file"/>' +
                '</div>' +
                '<div class="cell clearValue">' +
                '<button class="' + btnClass + '" >clear</button>' +
                '</div>' +
                '</div>' +
            ...
            container.find('div.clearValue button').on('click', function () {
                ...
                delete editor.frmData;
                container.find('.divUploadFile,img').remove();
            });
            ...
            function formDataDelete(frmData, name, index) {
                ...
                keep.forEach(function (value) {
                    return frmData.append(name, value);
                });
                if (keep.length === 0) {
                    delete editor.frmData;
                }
            ...
            function renameFile(filename) {
                var newFilename = prompt(window[editor.s.table.substring(1)].i18nEntries.renameFileTitle, filename);
                if (newFilename != null) {
                    return newFilename;
                } else {
                    return filename;
            ...
            //container.on("click", ".fileItem > .fas.fa-times", function (e) { //Event on "Remove File"
            container.on("click", ".faFileRemove", function (e) { //Event on "Remove File"
                var els = container.find('.faFileRemove');
                // PTE 2018.07.02 : adicionada por indicação de LEO para resolver problema de remoção de ficheiro sem submissão
                container.find('input[type=file]').val("");
                var idx = els.index(e.currentTarget);
                //$(this).parent('div').next('img').remove()
                $(this).closest(".divUploadFile").next('img').remove();
                $(this).closest('.divUploadFile').remove();
                formDataDelete(editor.frmData, 'upload[]', idx);
            });

            container.find('input[type=file]').on('change', function (e) { //Event on "Choose File"
                e.stopImmediatePropagation();
                if (!editor.frmData) {
                    editor["frmData"] = new FormData();
                }
                //var newFileName = renameFile(this.files[0].name);

                /*editor.frmData.append('upload[]', this.files[0]);
                editor.frmData.append('filenames[]', this.files[0].name);*/
                if (window[editor.s.table.substring(1)]['inRowDoc']) {
                    formDataDelete(editor.frmData, 'upload[]', 0);
                    if (!editor.frmData) {
                        editor["frmData"] = new FormData();
                    }
                    editor.frmData.append('upload[]', this.files[0]);
                    editor.frmData.append("inRowDoc", JSON.stringify(window[editor.s.table.substring(1)]['inRowDoc']));
                    container.find('.divUploadFile').remove();

                }
                /*editor.frmData.append('upload[]', this.files[0]);
                editor.frmData.append('filenames[]', this.files[0].name);*/
                container.append('<div class="divUploadFile"><span class="spanFileName">' + this.files[0].name + '<i class="fas fa-times faFileRemove pull-right"></i><div/>');
                //container.append('<div class="divUploadFile"><i class="fas fa-times quad-right-padding-4 faFileRemove"></i><span class="spanFileName badge badge-light">' + this.files[0].name + '<div/>');
            });
            return container;
        }
...
// types.
        ...
        fieldTypes.hidden = {
            ...
            create: function (conf) {
                delete conf.label;
                conf._input = $('<input style="display:none"/>').attr($.extend({
