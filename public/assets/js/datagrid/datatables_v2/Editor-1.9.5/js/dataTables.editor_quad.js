	...
	multiGet: function ( id ) {
		...
		if ( id === undefined ) {
			var fieldVal = this.val();
                        ...
                        // QUADSYSTEMS PTE 2020.10.08 - bug no caso do SELECT não estava a obter o valor da lista apenas pelo this.val()
                        if ($("[id='" + this.s.opts.id + "']").is('select')) {
                            fieldVal = $("[id='" + this.s.opts.id + "']").val();
                        }
		...
		$.each( editFields, function ( idSrc, edit ) {
			...
			$.each( fields, function (name, field) {
				if ( edit.fields[ name ] && field.submittable() ) {
					...
					var manyBuilder = Array.isArray( value ) && name.indexOf('[]') !== -1 ?
