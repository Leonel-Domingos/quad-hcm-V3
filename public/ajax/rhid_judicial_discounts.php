...
<div class="row">
    <div class="col-xl-12">
        <div id="panel-1" class="panel">
            <div class="panel-hdr">
                <span class="widget-icon"> <i class="fas fa-balance-scale"></i></span>&nbsp;
...
<script>
    ...
    var  _user = "<?php echo @$_SESSION['utilizador']; ?>", 
        _profile = "<?php echo @$_SESSION['perfil']; ?>", 
        rhid_ = "<?php echo @$_SESSION['rhid'];?>",
        whereClause_ = " A.TP_DESPESA = 'B' "; //Despesas Judiciais

    $(document).ready(function () {

        /* CRUD + WORKFLOW */
        var nomeForm = '<?php echo json_encode($frm); ?>', continue_,
            tabelas = ['RHID_JUDICIAL_DISCOUNTS'];
                
        //IF (CRUD + WORKFLOW) problem -> EXIT
        continue_ = go_no_go ('<?php echo json_encode($wkf_error); ?>', _user, _profile);
        
        var conf_JUDICIAL = JSON.parse('<?php echo $frm_definitions; ?>'),
        ...
        valid_requirements (nomeForm, conf_JUDICIAL, tabelas, _user, _profile);

        //IF ACCESS to RH_IDENTIFICACOES is FALSE -> EXIT
        if ( !conf_JUDICIAL['RHID_JUDICIAL_DISCOUNTS']["access"] ) {
        ...
        var optionsRH_ID_CONTAS_CORRENTES = {
            ...
            "workFlow": conf_JUDICIAL['RHID_JUDICIAL_DISCOUNTS']["workflow"],
            ...
            "tableCols": [
                {
                    ...
                    "render": function () {
                        return RH_ID_CONTAS_CORRENTES.crudButtons(conf_JUDICIAL['RHID_JUDICIAL_DISCOUNTS']["crud"][0], conf_JUDICIAL['RHID_JUDICIAL_DISCOUNTS']["crud"][1], conf_JUDICIAL['RHID_JUDICIAL_DISCOUNTS']["crud"][2]);
                    }
                }
            ],
                    ...
                    wk.onmessage = function (event) {                
                        if (event.data === 'working') {
                            ...
                            if (event.data.status == 'OK') {
                                ...
                                var val_pago = parseFloat(event.data.data[0]['TOT_PAGO']).toFixed(2),
                                    ...
                                    if (perc_pago ) {
                                        perc_pago = '&nbsp;<span class="label label-light-blue quad-pad-5 quad-right">' + perc_pago + '%</span>';
                                    } else {
                                        per_pago = '';
                                    }
        ...
        var optionRH_ID_DET_CONTAS_CORRENTES = {
            ...
            "dependsOn": {
                "RH_ID_CONTAS_CORRENTES": { //External object key mapping( object key : external key)                    
                    "ID": "ID",
                    "EMPRESA": "EMPRESA",
                    "RHID": "RHID",
            ...
            "tableCols": [
                {
                    ...
                    "className": "toBottom toCenter",
                    "render": function () {
                        return RH_ID_DET_CONTAS_CORRENTES.crudButtons(false,true,false);
                    }
                }
            ],
            "validations": {
